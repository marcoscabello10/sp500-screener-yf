...
## 🔴🔴 EL BUG MÁS CARO DEL MOTOR B: LA MATRIZ ESTABA DESALINEADA (03/09/2026)

### Cómo apareció

Una prueba que venía en verde empezó a fallar sola después de que Marcos
corriera los bots. `prueba-riesgo` dejó de detectar que **XOM y CVX son una
sola apuesta**. Medido: la correlación entre las dos petroleras más grandes de
Estados Unidos daba **0,037**.

Eso no es un umbral mal puesto. Eso es imposible.

### La causa

`retornosDe()` saltea los `null` —los días en que un papel no cotizó— y
devuelve un vector más corto. Después el código tomaba **los últimos N retornos
de cada papel y los pareaba por POSICIÓN**:
...
**Un solo día de corrimiento destruye la correlación entera**: cada retorno de
XOM se comparaba contra el de CVX del día siguiente.

Lo peor: el comentario del código anterior decía *"sin esto, dos papeles con
distinta cantidad de días se compararían sobre períodos distintos"*. **El
problema estaba visto.** Pero recortar desde el final no lo arregla, porque el
hueco está en el MEDIO, no al principio.

### Cuánto abarcaba

Medido sobre el snapshot real: **85 de 652 papeles (13%) tienen una cantidad de
días distinta de la mayoría** — 68 de ellos por exactamente un día.

| par | como estaba | alineado |
|---|---|---|
| **XOM–CVX** | **0,037** | **0,816** |
| AAPL–MSFT | 0,347 | 0,347 |
| WFC–JPM | 0,738 | 0,738 |
| KO–PEP | 0,611 | 0,612 |
| XOM–COP | 0,801 | 0,800 |

Los pares que coincidían en cantidad de días daban bien — **y por eso el bug
sobrevivió**: casi todas las pruebas usaban papeles que coincidían.

Cualquier cartera que mezclara uno de esos 85 con el resto tenía la matriz
corrompida, y con ella **todo el Motor B**: el peso objetivo por paridad de
riesgo, los pares correlacionados, el beta contra el índice y el ranking de
candidatos por delta de volatilidad.

### El arreglo

Los retornos se calculan sobre un **ÍNDICE COMÚN DE FECHAS**: los días en que
TODOS los papeles del análisis tienen precio. Así el retorno número 17 de un
papel y el 17 de otro son, por construcción, del mismo día.

- `indiceComun(series, desde, hasta)` — la intersección.
- `retornosEn(serie, idx)` — retornos sobre ese eje.
- `parAlineado(a, b, idx)` — para el benchmark y los candidatos, que **no**
  participan del índice de la cartera: si participaran, agregar un candidato
  cambiaría los números de la cartera y *"cuánto mejora si entra"* mediría dos
  cosas a la vez.

Si la intersección queda corta, se saca al papel con menos historia propia
—**nombrándolo**, como todo lo que se saca— y se reintenta. Sin eso, un papel
recién listado le arruinaría la ventana a los otros catorce.

`simular()` tenía el bug idéntico y se arregló igual.

### Y la prueba tenía el mismo bug

El "cálculo de control" de `prueba-riesgo.cjs` —que existe justamente para
verificar el módulo con una implementación aparte— **hacía `filter(x => x !=
null)` y pareaba por posición**. O sea: reproducía el error del código que
controlaba, y los dos coincidían en verde mientras los dos estaban mal.

> Un control que repite el error del código que controla no controla nada.

Ahora el control arma su propio eje común, escrito distinto. Y se agregó una
**prueba de regresión que no depende de ningún papel ni de ningún umbral de
mercado**: dos series idénticas salvo por un día que a una le falta. La
correlación real es 1,0; con el método viejo da 0,881. Es aritmética, así que
si el bug vuelve, esa prueba lo caza sin ambigüedad.

---

## 🚦 LA COMPUERTA DE DATOS — un solo lugar que diga "con esto no alcanza" (03/09/2026)
...
```
  metricas_usadas por posicion       "4/6"  ¿y eso alcanza?
  riesgo.cobertura_del_calculo_pct    87%   ¿de que? ¿de las acciones?
  cartera.cobertura_analizada_pct     71%   otra cosa distinta
  industrias.cobertura_pct            60%   otra mas
  nivel: completo | reducido                por informe individual
```

Cinco números que había que cruzar mentalmente para contestar la única pregunta
que importa —**¿alcanza para decidir?**— y que nadie cruzaba. El resultado era
el peor posible: el informe salía COMPLETO, con su tabla, su objetivo y su
prosa, sobre una cartera de la que sabíamos la mitad.

### Lo que agrega, y no es un sexto número

`suficienciaDeDatos()` devuelve un veredicto (`completo` / `parcial` /
`insuficiente`), un booleano (`puede_decidir`) y —lo único accionable de
todo esto— **`no_se_puede_afirmar`: la lista de frases que el informe NO tiene
derecho a decir con los datos que hay.**

> Un "87% de cobertura" no le dice nada a nadie. *"No se puede decir que la
> volatilidad de la cartera es 15,3%, porque ese número cubre el 87%"* sí.

### Se mide por PESO, no por conteo

Es la misma lección que este proyecto ya aprendió dos veces con los sectores.
Una posición del 1% sin puntaje no invalida nada; una del 40% sí. Contando
papeles las dos pesan igual, y entonces **un papel chico y raro apagaría el
informe entero**. Probado con el mismo papel al 2% y al 40%: el primero se
anota, el segundo calla la frase.

### No bloquea, y cuando todo está bien no dibuja nada

Un informe con datos parciales sigue siendo útil —a veces es todo lo que hay— y
esconderlo sería peor. Lo que no puede pasar es que se lea como si fuera
completo. Y **con datos completos el cartel no aparece**: un cartel que aparece
siempre se ignora siempre.

Viaja al modelo en 24 tokens (`datos` en el payload) con una regla nueva en el
prompt: *"LO PRIMERO QUE TENÉS QUE MIRAR"*. Y al texto del cliente solo cuando
`puede_decidir` es falso — un texto que arranca disculpándose por los datos
cuando los datos están bien es peor que no decir nada.

---

## 📈 FASE D — EL P/E CONTRA SU PROPIA HISTORIA (03/09/2026)

### La pregunta que faltaba

Todo el puntaje de este proyecto es un **percentil dentro del sector**: *"Apple
cotiza a 30x contra una mediana de 24x en Technology"*. Eso contesta *¿está cara
comparada con sus pares?* y no contesta *¿está cara para lo que ella suele
valer?*

Son preguntas distintas y las respuestas se contradicen seguido: un papel puede
ser **el más caro de su sector y estar en el punto más barato de su propia
historia** porque el sector entero se abarató. La segunda lectura es la que ve
una re-calificación y la que caza una trampa de valor.

### Sale gratis

Ninguna fuente nueva. Los dos ingredientes ya estaban:

- `historico_precios.json` — 6,7 años de cierres, que baja el Motor B;
- `historico.series.eps` — el EPS diluido por año fiscal, de EDGAR, auditado.

**Cero llamadas, cero tokens, cero API.**

### Las tres decisiones que lo hacen honesto

**1. El lag de 75 días.** Un balance anual no se conoce el día que cierra el
ejercicio: se publica dos o tres meses después. Usarlo desde el cierre sería
mirar el pasado con información que ese día nadie tenía, y el P/E histórico
saldría sistemáticamente más barato de lo que fue.

**2. El percentil por punto medio.** Contar solo los menores estrictos parece lo
mismo y no lo es: un papel cuyo P/E casi no se movió tiene la mayor parte de la
serie EMPATADA con el valor de hoy, y eso daba **percentil 0 — "en la parte más
barata de su historia"— cuando la verdad es que está exactamente donde
siempre estuvo**. Es la clase de error que no rompe nada y dice lo contrario.

**3. La guarda de dispersión.** Si entre el cuartil 25 y el 75 hay menos de un
5% de diferencia, el papel cotizó siempre al mismo múltiplo y *"está en su
percentil 90"* es ruido. Se dice que no hay lectura en vez de inventar una.

Y los dos P/E se muestran juntos con la aclaración de por qué difieren: el de
la tabla es **TTM**, este usa el **EPS anual reportado a la SEC**. Sin decirlo,
uno de los dos parece un error.

### Se carga a pedido

El histórico pesa ~9 MB y un informe individual no lo necesita para nada más.
Se baja al primer clic — y si el usuario ya vio una cartera, ya está en memoria
y el clic es instantáneo, porque `cargarHistorico()` cachea a nivel de módulo.

---

## 🇦🇷 LOS CÓDIGOS DE BYMA: EL COMENTARIO MENTÍA (03/09/2026)

Marcos: *"IRSA en el NYSE es IRS pero en el mercado local argentino es IRSA"*.

El mapeo de `ALIAS_LOCALES` **siempre estuvo bien**. El que mentía era el
comentario: afirmaba que IRSA usaba el mismo código en las dos plazas y lo
listaba entre los que "funcionan solos". Es peor que un bug de código, porque
es lo que se lee para decidir si hace falta tocar algo.
...
**Y ahora se usa de verdad.** Antes `resolver_alias` era una función de Python
que nadie llamaba desde el navegador: el Excel de un cliente argentino con
"YPFD" seguía sin encontrar el papel, **sin error y sin aviso**, como si hubiera
escrito cualquier cosa.

- El diccionario **viaja en `informe_detalle.json`** (`alias_locales`), no
  duplicado en JavaScript: una lista en dos lenguajes se desincroniza, y esta
  ya se equivocó una vez.
- `resolverTicker()` en `universo.js` **saca el sufijo `.BA` ANTES de buscar el
  alias**. Al revés, `YPFD.BA` no encontraría `YPFD` y el papel se perdería —
  justo el síntoma que esto viene a evitar.
- El Selector guarda `tickerOriginal` cuando difiere, para poder mostrar
  *"aparece como YPFD en tu Excel"*.

**Ecogas** (`ECOG` → `ECOG.BA`) entró al panel líder este año, no tiene ADR y
ahora está entre los `SOLO_MEDIBLES`. Son 11.
...
```
src/informe/riesgo.js        🔴🔴 EL BUG MAS CARO DEL MOTOR B: los retornos se
                             pareaban por POSICION, no por FECHA. XOM-CVX daba
                             0,037 de correlacion en vez de 0,816. Afectaba al
                             13% de los papeles y con ellos a TODO el Motor B
                             + indiceComun() / retornosEn() / parAlineado()
                             + el benchmark y los candidatos, al mismo eje
                             + simular() tenia el bug identico
                             + los que cotizan en pesos, fuera tambien de simular
src/informe/cartera.js       suficienciaDeDatos(): la compuerta, con umbrales
                             nombrados y medida por PESO, no por conteo
                             + `datos` al payload (24 tokens)
                             + datos_insuficientes a los hechos del cliente
src/informe/valuacionHistorica.js  NUEVO — fase D: el P/E contra su propia
                             historia. Lag de 75 dias, percentil por punto
                             medio, guarda de dispersion
src/informe/universo.js      resolverTicker(): alias + sufijo .BA
src/informe/Selector.jsx     el Excel con codigos de BYMA encuentra el papel
                             + tickerOriginal cuando difiere
src/informe/AppInforme.jsx   lee alias_locales y se lo pasa al Selector
src/informe/Cartera.jsx      <CompuertaDeDatos> arriba de todo
src/informe/Informe.jsx      <ValuacionHistorica>, que se carga a pedido
local_bot/cedears_informe.py 🔴 el comentario de IRSA mentia + ECOG (Ecogas)
                             + resolver_alias aguanta el sufijo .BA
local_bot/fetch_informe.py   alias_locales viaja en informe_detalle.json
api/informe.py               `datos` en la whitelist + la regla en el prompt
                             + datos_insuficientes en el prompt del cliente
                             + estimador re-medido: 1300 + 175n
test/prueba-datos.cjs        NUEVO — 33 comprobaciones
test/prueba-valuacion.cjs    NUEVO — 49 comprobaciones
test/prueba-riesgo.cjs       🔴 el control tenia el MISMO bug + regresion que
                             no depende de ningun papel real (+4)
test/prueba-excel.cjs        +9: los codigos de BYMA
test/test_universo_arg.py    +13: el sufijo, IRSA, Ecogas
test/test_tesis_cartera.py   claves opcionales + MEDIDO re-medido
test/medir_payload.py        el bloque `datos` entra a la medicion
PROMPT_CARTERA.txt           regenerado (3.167 tokens)
CONTEXTO_INFORME_AVANZADO.md
```

Dieciocho suites: 494 comprobaciones en JS + cinco suites de Python.
